...
To compute the number of business days between two dates bizdays uses these two indexes to handle the different situations where the arguments `from` and `to` are given nonworking days.
...
The devil is when we work with financial calendars.
...
All strange situations appear when non business days are passed as arguments to `bizdays` and `offset` functions.
...
But, the non sense here is the use of non business days as arguments when working with financial calendars.
